<script lang="ts">
	...
	import { session, isTauriEnv, loadSample, openFile, loadFilePath } from "$lib/session.svelte";
	...
	let lastConnectedCwd = $state<string | null>(null);
	...
	// Auto-reconnect: when a live connection drops for a non-manual reason (pi
	// restarted in the same project — new port, new session id, same cwd), find
	// the unique session whose cwd matches and reconnect automatically.
	// Cleared on any deliberate exit (Disconnect, Demo, Claude Code) so it never
	// fires after an intentional departure.
	$effect(() => {
		if (!lastConnectedCwd) return;
		if (live.status !== "idle") return;
		const candidates = discovery.sessions.filter(s => s.cwd === lastConnectedCwd);
		if (candidates.length !== 1) return; // ambiguous or gone — let user choose
		selectAndConnect(candidates[0]);
	});
	...
	function selectAndConnect(s: SessionEntry): void {
		...
		lastConnectedCwd = s.cwd;
		connectLive(s.port);
	}

	function handleDisconnect(): void {
		lastConnectedCwd = null; // user chose to leave — suppress auto-reconnect
		disconnectLive();
	}
	...
	<div class="content">
		{#if session.store}
			...
			<div class="app">
				<header class="topbar">
					...
					<div class="nav-row">
						{#if live.status === "connected"}
							<button class="nav-btn" onclick={handleDisconnect}>
								<Icon name="x" size={13} />
								Disconnect
							</button>
						{:else if isTauriEnv}
						...
						<button
							class="nav-btn"
							class:nav-btn-active={activityOpen}
				...
				<div class="main" class:open={!!(selectedBlock || selectedGroup) || activityOpen}>
					<div class="canvas">
		...
		{:else}
			<div class="fallback">
				...
				{:else if !discovery.sessions.length}
					<p class="hint">
						No live pi sessions found. Start <code>pi</code> in a project, or dial a port directly.
					</p>
					<div class="port-row">
