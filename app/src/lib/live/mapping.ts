/*
 * mapping.ts — pi message ↔ wire block bridge, shared by the GUI and the extension.
 *
 * `linearize(messages)` converts pi's in-memory messages to `WireBlock[]`.
 * `blockId(m, i, partIndex)` assigns durable content-anchored ids used by both sides.
 * `applyPlan` is kept for historical/test use; the Conductor no longer uses it at runtime.
 */
import type { WireBlock } from "./protocol";
import type { Block } from "../engine/types";

/** One in-place fold instruction (kept for applyPlan / tests). */
export interface FoldOp { id: string; digestText: string; }
/** One group-collapse instruction (kept for applyPlan / tests). */
export interface GroupOp { id: string; memberIds: string[]; summaryText: string; }
import { estTokens, BLOCK_OVERHEAD } from "../engine/tokens.ts";
...
export function applyPlan(messages: PiMessage[], ops: FoldOp[], groups: GroupOp[] = []): PiMessage[] {
	...
	const safeGroups = (groups ?? []).filter(
		(g) =>
			...
			g.memberIds.length &&
			g.memberIds.every((m: unknown) => typeof m === "string"),
	);
