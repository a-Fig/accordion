...
/** Browser-mode poll: fetch the session list from the extension's HTTP discovery server. */
async function browserPoll(): Promise<void> {
	if (_polling) return;
	_polling = true;
	try {
		const res = await fetch(`http://127.0.0.1:${DISCOVERY_PORT}/sessions`);
		if (!res.ok) return;
		const raw: unknown[] = await res.json();
		const now = Date.now();
		const live: SessionEntry[] = raw.filter(e => isLiveEntry(e, now));
		live.sort((a, b) => a.startedAt - b.startedAt);
		if (!sameSessions(discovery.sessions, live)) discovery.sessions = live;
		discovery.ready = true;
	} catch {
		/* discovery server not running — no sessions visible */
	} finally {
		_polling = false;
	}
}
...
export function startDiscovery(onFocus: (sessionId: string) => void): void {
	...
	if (isTauriEnv) {
		...
		void browserPoll();
		_timer = setInterval(() => void browserPoll(), POLL_MS);
	}
}
