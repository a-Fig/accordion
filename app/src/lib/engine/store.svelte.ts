...
import type { WireGroup, WireFoldDecision, SyncMessage } from "../live/protocol";
...
export class AccordionStore {
	...
	version = $state(0);
	/**
	 * When true the store is driven by the Conductor (live session). The local
	 * auto-folder is disabled and fold state comes exclusively from the extension
	 * via `applyLiveSnapshot`. User actions still work but are emitted over the
	 * WebSocket rather than applied locally — the snapshot from the next sync is
	 * the source of truth. Toggled by `liveClient.svelte.ts` on connect/disconnect.
	 */
	liveMode = $state(false);
	/** Conductor fold level per block id (0=full · 1=trim · 2=digest · 3=group member). Live mode only. */
	liveFoldLevels = $state<Record<string, 0 | 1 | 2 | 3>>({});
	/** Exact text the agent receives for each folded block. Live mode only. */
	liveFoldedDigests = $state<Record<string, string>>({});
	/**
	...
	digestOf(b: Block): string {
		// In live mode, show the exact text the agent receives (Conductor format:
		// ⟦tN⟧ body, not {#code FOLDED} body). Falls back to app-engine digest for
		// local/file sessions where the Conductor isn't running.
		if (this.liveMode && this.liveFoldedDigests[b.id]) return this.liveFoldedDigests[b.id];
		return digest(b);
	}
	...
	refold(): void {
		// In live-mirror mode the Conductor is the auto-folder; skip local refold.
		if (this.liveMode) { this.version++; return; }
	...
	// ---- live-mirror mode (Conductor is authoritative) ----------------------

	setLiveMode(on: boolean): void {
		this.liveMode = on;
		if (!on) {
			this.liveFoldLevels = {};
			this.liveFoldedDigests = {};
			for (const b of this.blocks) {
				b.override = null;
				b.by = null;
			}
	...
	/**
	 * Overwrite the store's fold/pin/group state from the Conductor's authoritative
	 * snapshot (received in every SyncMessage). Called only in live-mirror mode.
	 * Blocks in foldedBlockIds get override:"folded"/by:"conductor"; blocks in
	 * pinnedBlockIds get override:"pinned"/by:"conductor"; all others are cleared.
	 */
	applyLiveSnapshot(
		...
		const folded = new Set(foldedBlockIds);
		...
		this.groups = groups.map((g) => ({ id: g.id, memberIds: [...g.memberIds], folded: g.folded }));
		this.liveFoldLevels = foldLevels ?? {};
		this.liveFoldedDigests = foldedDigests ?? {};
		this.version++;
	}

	/**
	 * Stream Conductor decisions into `store.log` so the activity panel can render
	 * them. Called on every SyncMessage that carries decisions.
	 */
	recordConductorDecisions(decisions: WireFoldDecision[]): void {
		for (const d of decisions) {
			const actor: Actor = d.actor === "conductor" ? "conductor" : d.actor === "agent" ? "agent" : "you";
			const reason = Array.isArray(d.reason) ? d.reason.join(" · ") : d.reason;
			const detail = `${d.kind}${d.callId ? ` (${d.callId.slice(0, 8)})` : ""} · turn ${d.turn}${reason ? " · " + reason : ""}`;
			this.emit(actor, d.action, detail);
		}
	}
}
