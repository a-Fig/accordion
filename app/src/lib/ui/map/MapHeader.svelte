	...
	<div class="top">
		...
		<div class="ctl">
			...
			{#if live.status === "connected"}
				<span
					class="fold-arm on"
					title="Accordion Conductor is active — folds apply to the agent's context each turn"
				>
					<Icon name="activity" size={13} />
					<span class="fold-arm-dot" aria-hidden="true"></span>
					<span class="fold-arm-label">Folding: active</span>
				</span>
			{/if}
			...
			<label class="knob">
				...
				<input
					...
					oninput={(e) => {
						...
						store.setBudget(v);
						try { localStorage.setItem("accordion.budget", String(v)); } catch {}
					}}
